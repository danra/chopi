#!/usr/bin/env bash
#
# test/chopi-preflight.sh -- unit tests for preflight checks
#
...
# ---------------------------------------------------------------------------
. "$repo/.internal/chopi-preflight.sh"
set +euo pipefail
...
# ---------------------------------------------------------------------------
out="$(cd "$repo"        && env -u CHOPI_ALLOW_SELF ./.internal/chopi-preflight.sh 2>&1)"; st=$?
...
out="$(cd "$work" && env -u CHOPI_ALLOW_SELF "$repo/.internal/chopi-preflight.sh" --config "$cfg_inside" 2>&1)"; st=$?
assert_contains "$out" "is inside the workspace" "refuses a --config placed inside the workspace"
if [ "$st" -ne 0 ]; then ok "  -> and exits non-zero"; else bad "  -> should exit non-zero (got $st)"; fi

out="$(cd "$work" && "$repo/.internal/chopi-preflight.sh" --config sandbox.sh 2>&1)"
...
out="$(cd "$work" && "$repo/.internal/chopi-preflight.sh" --config "$work/sub/cfg.sh" 2>&1)"
assert_contains "$out" "is inside the workspace" "refuses a --config nested in a workspace subdir"

out="$(cd "$work" && CHOPI_ALLOW_SELF=1 "$repo/.internal/chopi-preflight.sh" --config "$cfg_inside" 2>&1)"
...
cfg_outside="$(mktemp)"
out="$(cd "$work" && "$repo/.internal/chopi-preflight.sh" --config "$cfg_outside" 2>&1)"
assert_not_contains "$out" "is inside the workspace" "a --config outside the workspace clears the overlap check"

out="$(cd "$work" && "$repo/.internal/chopi-preflight.sh" --config /no/such/dir/cfg.sh 2>&1)"; st=$?
